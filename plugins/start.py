...
async def start_command(client: Client, message: Message):
    ...
    if not await present_user(id):
        # If new user, add user to the database and send message to the channel
        try:
            await add_user(id)
        except:
            pass

        # Send message to the channel when a new user starts the bot
        user_profile_link = f"https://t.me/{message.from_user.username}"
        await client.send_message(-1002197279542, f"New user started the bot: {user_profile_link}")
        ...
        reply_markup = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton('🍁 Cʜᴀɴɴᴇʟ', url='https://t.me/DonghuaQuest')
                ]
            ]
        )
